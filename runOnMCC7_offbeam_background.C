...
//     treenc -> Add( ("/pnfs/uboone/persistent/users/aschu/MC_BNB_Cosmic/"+GeneratorName+"_"+Version+"_anatree.root").c_str() );
    ...
    for(const auto& TrackingName : TrackProdNameVec)
    {
        ...
        {
            // Open output file
            TFile* OutputFile = new TFile(("rootfiles/Hist_Cosmic_Background_Track_"+TrackingName+ "_Vertex_" + VertexingName + "_"+GeneratorName+"_"+Version+".root").c_str(),"RECREATE");
                ...
                if(potbnb > 0.0 || (GeneratorName != "data_bnb" && GeneratorName !="data_onbeam_bnb"))
                {
                    ...
                    if(flashtag)
                    {
                        ...
                        if(TrackCandidate > -1)
                        {
                            ...
                            if( FlashTrackDist(flash_zcenter[theflash], trkstartz[TrackCandidate], trkendz[TrackCandidate]) > flashwidth ) // Changed! Take all but the ones that match
                            {
                                if(FlashMatchFlag)
                                {
                                    EventsFlashMatched++;
